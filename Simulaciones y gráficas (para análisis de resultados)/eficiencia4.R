...
sigmacomida = 10 #sigma value for normal truncated eating time
...
while (lambdamic<6 & lambdacaf<6){
  ...
  while (w<MAX){
    ...
    mediaSEN1 = c(mediaSEN1, SUMA21/TM)
    mediaSEN2 = c(mediaSEN2, SUMA22/TM)
    TLMIC = c(TLMIC, total_llegadas11)
    TLMEN = c(TLMEN, total_llegadas12)
    TLBAR = c(TLBAR, total_llegadas13)
    TLSEN1 = c(TLSEN1, total_llegadas21)
    TLSEN2 = c(TLSEN2, total_llegadas22)
    TSMIC = c(TSMIC, total_servicio11)
    TSMEN = c(TSMEN, total_servicio12)
    TSBAR = c(TSBAR, total_servicio13)
    TSSEN1 = c(TSSEN1, total_servicio21+ifelse(N21>aforomax_SA, aforomax_SA, N21))
    TSSEN2 = c(TSSEN2, total_servicio22+ifelse(N22>aforomax_Caf, aforomax_Caf, N22))
    w = w + 1
  }
  intMIC = c(intMIC, mean(TSMIC)/mean(TLMIC))
  intMEN = c(intMEN, mean(TSMEN)/mean(TLMEN))
  intBAR = c(intBAR, mean(TSBAR)/mean(TLBAR))
  lambdamic = lambdamic + 0.1
